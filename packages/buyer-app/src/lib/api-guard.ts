 ...
 *
 * For JSON API routes behind SameSite cookies, checking Origin + Content-Type
 * is sufficient CSRF protection (no token needed).
 */
export function guardApiRoute(
  ...
  if (request.method !== "GET" && request.method !== "HEAD") {
    ...
    // In production, Origin must be present and match the host
    if (origin && host) {
      try {
